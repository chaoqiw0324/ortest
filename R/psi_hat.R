...
#' is linear regression model, MARS, random forest and XGBoost. If 
#' \code{y} is binary, the default method is linear regression model, MARS, random forest 
#' and XGBoost.
#' This model is tested whether \code{x} and \code{y} is independent conditional on
#' \code{S}. The final result is the test statistics and standard error.
#' @return Test statistics and standard error of the test. 
...
psi_hat_sl <- function(y, x, S=c(), subset = NULL, out_bin = TRUE, exp_bin = FALSE,sl=NULL,cross_fitting = FALSE,kfolds=5){
  ...
  sl_bin <- c("SL.glm","SL.earth","SL.ranger") #"SL.mean"
  sl_gau <- c("SL.glm","SL.earth","SL.ranger")
  if(! is.null(sl)){
...
#' Super-Learning Based Conditional Independence Test With Correction
#'
#' We test whether \code{x} and \code{y} are conditionally associated, given
#' \code{S} using a generalized linear model. 
#' 
...
psi_hat_sl_cr <- function(y, x, S=c(), subset = NULL, out_bin = TRUE, exp_bin = FALSE,sl=NULL,cross_fitting = FALSE,kfolds=5){
  ...
  sl_bin <- c("SL.glm","SL.earth","SL.xgboost") #"SL.mean"
  ...
  if (cross_fitting==FALSE) {
    ...
    for (i in 1:kfolds) {
      ...
      if(out_bin && exp_bin){
        ...
        se <- as.numeric(sqrt(sandwich / (n/kfolds))) 
        result <- c(res,se)
        if (se > 10*res|is.infinite(se)) {
          message(paste("Fold", i, "reported abnormal SE (", round(se, 4),"). Result may be unstable. Using glm to refit the model"))
          h.dag <- 0.25 ## probability f.dag(y|S) = g.dag(x|S) = 0.5, i.e., y ~ x ~ Bernoulli(0.5)
          X_out_train  <- as_tibble(X_out[train_indices,])
          colnames(X_out_train) <- colnames(X_out)
