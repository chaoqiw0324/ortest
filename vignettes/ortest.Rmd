...
if (has_pcalg) {
  V <- colnames(data)
  pc.fit <- pcalg::pc(
    suffStat = list(dat = data, method="linear", sl=NULL,
                    cross_fitting=FALSE, kfolds=5, two_way=FALSE, three_way=FALSE),
    indepTest = ortest,
...
```{r,echo = F,eval=FALSE,message=FALSE,warning=FALSE,error=FALSE,fig.cap = "PC Plot showing the potential relationship"}
plot(pc.fit, main = "")
```
