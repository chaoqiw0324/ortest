...
In this vignette, we will introduce the main functions provided in  __`ortest`__ package, including `psi_hat_linear()`, `psi_hat_sl()`, `basic_function()`,`multi_level()` and `OR_test()`.
...
```{r,message=FALSE,warning=FALSE,error=FALSE,fig.cap = "PC Plot showing the potential relationship"}
plot(pc.fit)
```
